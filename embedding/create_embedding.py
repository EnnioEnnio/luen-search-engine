...
def main():
    parser = argparse.ArgumentParser(
        description="Generate embeddings for MS MARCO dataset"
    )
    parser.add_argument(
        "--data", type=str, required=True, help="Path to input TSV file"
    )
    parser.add_argument(
        "--output-dir", type=str, required=True, help="Directory to save embeddings"
    )
    parser.add_argument(
    ...
    parser.add_argument(
        ...
        default=256,
        help=(
            "Max sequence length (default: 256 for local performance. "
    ...
    parser.add_argument(
        ...
        type=int,
        default=64,
        help="Embedding dimension (default: 64 for Matryoshka). Model default is 768.",
    )
    ...
    model = AutoModel.from_pretrained(
        "nomic-ai/nomic-embed-text-v1.5", trust_remote_code=True, dtype=torch.float16
    )
    model.to(device)
    model.eval()
